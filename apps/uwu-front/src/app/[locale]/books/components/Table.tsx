...
import { BOOKS, Book, Lend, TOTAL_COUNT } from '@/src/Constants';
...
function Table() {
  ...
  const totalBooks = responseTotalCount.data.totalCount;

  const { data } = useSuspenseQuery(BOOKS, {
    variables: {
      search: search,
      offset: offset,
      limit: itemsPerPage,
  ...
  const handleChangePage = (newOffset: number) => {
    if (newOffset < 0 || newOffset >= totalBooks) return;
    setOffset(newOffset);
  };
        ...
        <div className="flex-none overflow-x-auto shadow-md sm:rounded-lg">
              ...
              {books.map((book) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 capitalize">
                    {t(`TableBooks.${book.area}`).toLowerCase()}
                  </td>
                  {/* <td className="px-6 py-4">{book.inventory}</td> */}
                  <td
                    className={
                      book.lend === Lend.PRESTADO
                        ? 'font-bold text-black'
                        : 'font-bold text-green-500'
                    }
                  >
                    {book.lend === Lend.PRESTADO
                      ? t('TableBooks.borrowed')
                      : t('TableBooks.available')}
                  </td>
          ...
          >
            <span className="text-sm font-normal text-white">
              ...
              of <span className="font-semibold text-white">{totalBooks}</span>
            </span>
